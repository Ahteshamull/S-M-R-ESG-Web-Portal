...
export default function WaterDashboardPage() {
  ...
  const { data: waterLogs = [], refetch: refetchWaterLogs } =
    useGetWaterLogsQuery({});
  ...
  const filteredLogs = useMemo(() => {
    const list = waterLogs.filter((log: any) => {
      const matchYear =
        selectedYear === "All" || (log.year ? log.year === selectedYear : true);
      const matchMonth =
        selectedMonth === "All" ||
        log.month?.toLowerCase() === selectedMonth.toLowerCase();
      const matchSearch =
        searchQuery.trim() === "" ||
        log.month?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (log.goodsType &&
          log.goodsType.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const kpis = useMemo(() => {
    const totalWithdrawal = filteredLogs.reduce(
      (sum, log) => sum + (Number(log.totalWithdrawal) || 0),
      0,
    );
    const totalProdKg = filteredLogs.reduce(
      (sum, log) => sum + (Number(log.productionKg) || 0),
      0,
    );
    const totalGroundwater = filteredLogs.reduce(
      (sum, log) => sum + (Number(log.sources?.groundwater) || 0),
      0,
    );
    const totalRain = filteredLogs.reduce(
      (sum, log) => sum + (Number(log.sources?.rainWater) || 0),
      0,
    );
    const totalRecycle = filteredLogs.reduce(
      (sum, log) => sum + (Number(log.sources?.recycleWater) || 0),
      0,
    );

    const waterIntensity =
      totalProdKg > 0
        ? ((totalWithdrawal * 1000) / totalProdKg).toFixed(1)
        : "0.0";
    const gwRatio =
      totalWithdrawal > 0
        ? ((totalGroundwater / totalWithdrawal) * 100).toFixed(1)
        : "0.0";
    const altRatio =
      totalWithdrawal > 0
        ? (((totalRain + totalRecycle) / totalWithdrawal) * 100).toFixed(1)
        : "0.0";
  ...
  const deptSummary = useMemo(() => {
    ...
    filteredLogs.forEach((l: any) => {
      ...
      drinking +=
        Number(l.departments?.domesticDrinking) ||
        Number(l.departments?.domesticDiningDrinking) ||
        0;
      washrooms +=
        Number(l.departments?.domesticWashrooms) ||
        Number(l.departments?.domesticToiletCanteen) ||
        0;
  ...
  const selectedAreaVolume = useMemo(() => {
    switch (selectedProcessType) {
      case "Dyeing":
        return deptSummary.dyeing;
      case "Washing":
        return deptSummary.washing;
      case "Printing":
        return deptSummary.printing;
      case "Utility":
        return deptSummary.boiler + deptSummary.cooling;
      case "Garments (Cut to Pack)":
      case "Garments":
      case "Cut to Pack":
        return deptSummary.garmentsTotal;
      default:
        return kpis.totalWithdrawal;
    }
  ...
  const fabricBenchmarks = useMemo(() => {
    const getIntensityFor = (type: "denim" | "fabric_washing" | "garment") => {
      const typeLogs = filteredLogs.filter(
        (l: any) =>
          l.goodsType && l.goodsType.toLowerCase() === type.toLowerCase(),
      );
      const totalW = typeLogs.reduce(
        (sum: number, l: any) => sum + (Number(l.totalWithdrawal) || 0),
        0,
      );
      const totalKg = typeLogs.reduce(
        (sum: number, l: any) => sum + (Number(l.productionKg) || 0),
        0,
      );
      if (totalKg > 0) return Number(((totalW * 1000) / totalKg).toFixed(1));
      if (selectedGoodsType === type && kpis.waterIntensity > 0)
        return kpis.waterIntensity;
      return 0;
    };
  ...
  const fabricPerformanceList = useMemo(() => {
    const list = [
      ...
      { fabric: "CVC Blend", actual: 95, target: 95 },
      {
        fabric: "Denim",
        actual:
          selectedGoodsType === "denim" && kpis.waterIntensity > 0
            ? Math.round(kpis.waterIntensity)
            : 35,
        target: 35,
      },
      {
        fabric: "Fabric Washing",
        actual:
          selectedGoodsType === "fabric_washing" && kpis.waterIntensity > 0
            ? Math.round(kpis.waterIntensity)
            : 41,
        target: 41,
      },
      {
        fabric: "Garments",
        actual:
          selectedGoodsType === "garment" && kpis.waterIntensity > 0
            ? Math.round(kpis.waterIntensity)
            : 46,
        target: 46,
      },
    ...
    return list.filter((f) =>
      f.fabric.toLowerCase().includes(selectedFabricFocus.toLowerCase()),
    );
  ...
  const displayTotalLiters = useMemo(() => {
    if (selectedProcessType === "All Departments") {
      return kpis.totalWithdrawal > 0
        ? (kpis.totalWithdrawal * 1000).toLocaleString()
        : "0";
    }
    return selectedAreaVolume > 0
      ? (selectedAreaVolume * 1000).toLocaleString()
      : "0";
  }, [selectedProcessType, selectedAreaVolume, kpis.totalWithdrawal]);
  ...
  const activeTelemetry = useMemo(() => {
    ...
    const baseUsage =
      selectedAreaVolume > 0 ? selectedAreaVolume * 1000 : 150000;
    ...
    if (
      ...
      return {
        ...
        trendData: dailyTrendData.map((d) => ({
          ...d,
          value: Math.round(d.value * 0.07),
        })),
        comparisonTitle:
          "1. Production & Finishing vs. 2. Domestic & Sanitation (m³)",
        comparisonSubtitle:
          "Steam boiler consumption, workers' drinking water, washrooms, and canteen audits",
        comparisonData: [
          {
            name: "Steam Boiler (Finishing)",
            actual: deptSummary.garmentsSteam,
            target:
              deptSummary.garmentsSteam > 0
                ? Math.round(deptSummary.garmentsSteam * 0.9)
                : 0,
            unit: "m³",
          },
          {
            name: "Workers' Drinking Water",
            actual: deptSummary.drinking,
            target: deptSummary.drinking,
            unit: "m³",
          },
          {
            ...
            target:
              deptSummary.washrooms > 0
                ? Math.round(deptSummary.washrooms * 0.9)
                : 0,
            unit: "m³",
          },
          {
            name: "Canteen Services",
            actual: deptSummary.canteen,
            target: deptSummary.canteen,
            unit: "m³",
          },
        ],
        ...
        effluentData: [
          ...
          },
          {
            ...
            inflow: deptSummary.drinking,
            treated: deptSummary.drinking,
          },
          {
            name: "Washrooms Line",
            inflow: deptSummary.washrooms,
            treated: Math.round(deptSummary.washrooms * 0.95),
          },
          {
            name: "Canteen Wash",
            inflow: deptSummary.canteen,
            treated: deptSummary.canteen,
          },
        ],
        ...
        sensorReadings: [
          {
            ...
            flow:
              deptSummary.garmentsSteam > 0
                ? Number((deptSummary.garmentsSteam / 30).toFixed(1))
                : 0,
          ...
          },
          {
            ...
            flow:
              deptSummary.drinking > 0
                ? Number((deptSummary.drinking / 30).toFixed(1))
                : 0,
          ...
          },
          {
            ...
            flow:
              deptSummary.washrooms > 0
                ? Number((deptSummary.washrooms / 30).toFixed(1))
                : 0,
          ...
          },
          {
            ...
            flow:
              deptSummary.canteen > 0
                ? Number((deptSummary.canteen / 30).toFixed(1))
                : 0,
        ...
        extraCards: [
          {
            ...
            subtitle:
              "Garments ironing, finishing tunnels and packaging steam line",
            badge: "Production",
          },
          {
            ...
            subtitle:
              "Safe RO drinking water for workers and hygiene sanitation lines",
            badge: "Sanitation",
          },
          {
            ...
            subtitle:
              "Daily hygienic cooking, kitchen and washing facilities water",
            badge: "Canteen",
          },
        ],
      };
    ...
    return {
      ...
      title:
        selectedProcessType === "Washing"
          ? "4. Facility Wise Water Consumption"
          : `4. ${selectedProcessType} Water Consumption`,
      subtitle:
        selectedProcessType === "Washing"
      ...
      comparisonTitle:
        selectedProcessType === "Washing"
          ? ""
          : `${selectedProcessType.toUpperCase()} PERFORMANCE BENCHMARKS`,
      comparisonSubtitle:
        selectedProcessType === "Washing"
          ? ""
          : "Standard benchmarks vs actual industrial performance",
      comparisonData:
        selectedProcessType === "Washing"
          ? []
  ...
  const handleExportExcel = () => {
    ...
    const valBoiler = filteredLogs.reduce(
      (sum, l) =>
        sum + (l.departments?.utilityBoilerSteam || l.totalProduction || 0),
      0,
    );
    const valY = 65.45;
